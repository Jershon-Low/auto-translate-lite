...
- **Cache-fill failure** (Gemini error while filling `missingEntries`): identical fallback shape to today — `translateWithFallback`'s existing retry-without-sermon-cache pattern applies to `verifyTranslationsWithRetry` already; if translation itself fails, `ensureBacklogCached` should catch and treat all `missingEntries` as failed-safe (cache each as English fallback, same as `outgoing = line.english` in the live path), logged via the existing `logEvent('error', ...)` pattern, so a subscribe never hangs or throws on the client. Once a line is cached this way it is no longer "missing" (per the cache-hit check in step 4), so — unlike the live path, where the next segment gets a fresh translation attempt — a backlog line that fails during a cold fill stays pinned to its English fallback for the rest of the session; it is never retried by a later subscribe. This is the deliberate trade-off that avoids a thundering-herd retry loop on a sustained Gemini outage (see the next bullet); recovering a specific failed line would require caching failures as a distinct sentinel rather than as the English fallback, which is out of scope here (see Future Extensions).
...
- Retrying a backlog line that failed during a cold fill (currently pinned to its English fallback for the rest of the session, per Error Handling above) — would require a distinct "failed" sentinel in the cache, separate from a legitimate English-fallback value, so a later subscribe could tell the difference and retry.
