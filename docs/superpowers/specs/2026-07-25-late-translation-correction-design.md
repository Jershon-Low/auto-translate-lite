...
- **An admin remove-then-reinstate with *unchanged* text can produce two terminal messages for one `id`.** The third guard in `scheduleCorrection` (§2) only catches a reinstate that *edits* the text; a reinstate that restores the exact original English passes `line.english === shedEnglish` and lets the original in-flight correction proceed alongside whatever correction the reinstate itself schedules. This technically violates the "exactly one terminal message" invariant in letter, but is harmless in effect: both messages carry a valid translation of the same English, both clear the waiting state, the client is idempotent under either ordering, and the viewer already saw a `line-removed` / `caption-inserted` reset in between. Not worth a fourth guard for a same-text remove-and-restore.
