...
The pipeline has to sustain the speaker's pace indefinitely, so no LLM call sits on a serial path:

- **Transcription checks run concurrently**, with the results re-ordered before broadcast. A segment reserves its ordered slot the moment it arrives, so a slow check delays only its own line — never the ones behind it. (It used to: one serial chain meant a check slower than the gap between sentences accumulated backlog with no way to drain, and a live session once ran 6 minutes behind.)
- **Tiered LLM admission** — the transcription check runs at `critical` priority, ahead of translation (`live`) and on-subscribe backfill (`backlog`), so the cheap call every caption depends on never queues behind the expensive one.
- **Back-pressure at both stages** — a translation that misses `MAX_PUBLISH_LAG_MS` publishes as English and upgrades in place when it lands (within `MAX_CORRECTION_LAG_MS`); a transcription check that misses it publishes unchecked.
- **Watch `verify_lag_high`** in `events.log` — it is edge-triggered on the age of the oldest segment still awaiting its check, and is the metric that surfaces this class of problem.
...
- **Fail-safe fallback, not fail-open** — if a translation is flagged unsafe, the safety check itself fails, or the translation for a language never comes back at all, the viewer sees the **English original** instead of a suspect translation. The system never ships a translation it couldn't verify, and every active language always receives exactly one caption per line — a language the model omits falls back to English (logged as `translation_missing_language`) rather than leaving that viewer's line hanging. A flagged *transcript* line has no safe fallback to show — it *is* the source — so it's suppressed entirely from every viewer, live and backlog. It still shows up, marked as flagged, on the capture page's read-only feed (for operational awareness) and on the review page, where an operator can reinstate a corrected version or remove it outright; it just never reaches the congregation until then.
- **Bounded, not unbounded, waiting** — the transcription check is the one call every caption in every language is blocked behind, so it is capped: if it hasn't returned within `MAX_PUBLISH_LAG_MS`, the line publishes **unchecked** rather than holding the caption feed (logged as `transcription_verify_timeout`). This is a deliberate trade — a checker that is down or hung would otherwise take the whole congregation's captions down with it, and a late verdict is ignored rather than yanking a line back off screen mid-read. See [the design note](docs/superpowers/specs/2026-07-26-concurrent-transcription-verification-design.md).
